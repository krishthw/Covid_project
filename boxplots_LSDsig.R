...
df_data<-data.table::fread("/Users/krishanthi/Documents/Liyanage_lab/Liyanage_data/Covid-3/IN2-final/Manuja_made2/Isoplexis Codeplex final- February 10th 2021.csv")
dff<-df_data%>% select(c(1,2,5,6:length(colnames(df_data))))
...
dff$Diabetes[dff$Diabetes == "CTRL"] <- "" # Remove CTRL label
...
# important
# if columns with NA present, exsecute *** to get statistics and boxplots. Not for correlations
names(which(colSums(is.na(dff))>0))
df<-dff %>% select(Group, names(nonzero)) # Group and non-zero columns
# ***
df<-dff %>% select(Group, names(nonzero), names(which(colSums(is.na(dff))>0))) # non-zero columns and columns with NA
...
write.csv(kruskaltest,'CovidFinalfeb/CytokineCodeplex/Reports/codeplex_kruskal.csv')
...
write.csv(ANOVAtest,'CovidFinalfeb/CytokineCodeplex/Reports/codeplex_anova.csv')

joinsig<-inner_join(kruskalsig, ANOVAsig)
write.csv(joinsig,'CovidFinalfeb/CytokineCodeplex/Reports/codeplex_anovakruskal.csv')
...
colnames(tukeyLSD_dfsig)<-c("marker","Tukey-HD_ICU","Tukey-HD_RD","Tukey-ICU_RD",
                            "Fisher-HD_ICU","Fisher-HD_RD","Fisher-ICU_RD")
write.csv(tukeyLSD_dfsig,'CovidFinalfeb/CytokineCodeplex/Reports/codeplex_TukeyFisher.csv')
...
for (i in seq(1:(ncol(dfsig)-1))) {
  ...
  box_lsd<-ggplot(df1, aes(x = get(colname1), y = get(colname2 )))+
    ...
    geom_violin(aes(colour=get(colname1),fill=get(colname1)),trim=F)+
    ...
    xlab("") + ylab("")+  theme(legend.position = "none")
  b<-box_lsd+ stat_summary(fun.data=mean_sdl,geom="pointrange", color="black",size=0.1,fun.args = list(mult = 1))
  # b<-box_lsd+stat_summary(fun=mean, geom="point", shape="-",size=10,color='black')
  print(b)
  ggsave(b,  filename = paste0("CovidFinalfeb/CytokineCodeplex/Boxplots/", colname2, ".png"),
         dpi = 300, type = "cairo",  width = 4, height = 6, units = "in")
}

# __________________________________________________________________________________________
#Boxplot with deceased ,Diabetes and FisherLSD test significance labeled 
...
for (i in seq(1:(ncol(dfsig)-1))) {
  ...
  box_lsd<-ggplot(df1, aes(x = get(colname1), y = get(colname2 )))+
    ...
    geom_point(data=dlive,position=position_jitter(seed=1),
               shape=19,size = 5,color="orange")+
    geom_point(data=notddead,position=position_jitter(seed=1), 
               shape=19,size = 5,color="#585858")+
    geom_point(data=ddead,position=position_jitter(seed=1), 
    ...
    xlab("") + ylab("")+  theme(legend.position = "none")
  print(box_lsd)
  ggsave(box_lsd,  filename = paste0("Delete/",colname2, ".png"),
         dpi = 300, type = "cairo",  width = 4, height = 6, units = "in")
}
dev.off()
...
for (i in seq(1:(ncol(dfsig)-1))) {
  ...
  box_lsd<-ggplot(df1, aes(x = get(colname1), y = get(colname2 )))+
    ...
    xlab("") + ylab("")+  theme(legend.position = "none")
  print(box_lsd)
  ggsave(box_lsd,  filename = paste0("CovidFinalfeb/Innate/Boxplots/",colname2, ".png"),
       dpi = 300, type = "cairo",  width = 4, height = 6, units = "in")
}
dev.off()

# __________________________________________________________________________________________
# Boxplot with FisherLSD test significance labeled for Plasma data
...
for (i in seq(1:(ncol(dfsig)-1))) {
  ...
  box_lsd<-ggplot(df1, aes(x = get(colname1), y = get(colname2 )))+
    theme(panel.background = element_rect(fill="white"),
          ...
          axis.text.y = element_text(size=10,colour="black",face="bold")) +
    scale_x_discrete(limits=c("HD", "ICU","RD"),labels=c("HD" = "HD","ICU" = "ICU","RD"="RD"))+
    geom_boxplot(aes(colour=get(colname1),fill=get(colname1)),outlier.shape = NA,colour="black",alpha=0.9)+
    stat_pvalue_manual(lsd_p, hide.ns = T,size=6)+
    geom_jitter(position=position_jitter(width=.20, height=0), 
                aes(colour=get(colname1)),
                shape=19,size = 2,color="black",alpha=0.8)+
    scale_fill_manual(values = c("forestgreen","red2","cornflowerblue"))+
    xlab("") + ylab("")+  theme(legend.position = "none")
  print(box_lsd)
  ggsave(box_lsd,  filename = paste0("CovidFinalfeb/Plasma/Boxplots/", colname2, ".png"),
        dpi = 300, type = "cairo",  width = 4, height = 6, units = "in")
}
dev.off()
